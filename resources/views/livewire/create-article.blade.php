<div class="container vh-100 pt-5">
    ...
    <form wire:submit="store">
        ...
        <div class="">
            <button type="submit" class="btn btn-primary mt-5">{{__('ui.Createarticle')}}</button>
        </div>
        @if (!empty($images))
        <div class="row">
            <div class="col-12 text-center">
                ...
                <div class="row">
                    ...
                    <div class="imgPreviewCustom mx-auto" style="background-image: url('{{ $image->temporaryUrl() }}');"></div>
                        <button type="button" class="btn btn-danger mt-4" wire:click="removeImage({{$key}})">{{__('ui.delete')}}</button>
                    </div>
                    @endforeach
                </div>
            </div>
